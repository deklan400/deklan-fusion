...
def run_cmd(cmd: str, timeout: int = 300) -> subprocess.CompletedProcess:
    ...
    try:
        ...
        # fake CompletedProcess kalau timeout
...
async def create_swap(msg: Message, size_gb: int) -> None:
    ...
    # NOTE: pakai SWAPFILE_PATH (string), bukan Path object
    swap = shlex.quote(SWAPFILE_PATH)

    cmd = f"""
set -e
swapoff -a || true
rm -f {swap}
if ! fallocate -l {size_gb}G {swap}; then
  dd if=/dev/zero of={swap} bs=1G count={size_gb} status=progress
fi
chmod 600 {swap}
mkswap {swap}
swapon {swap}
...
if grep -q '^{SWAPFILE_PATH}' /etc/fstab 2>/dev/null; then
  sed -i 's|^{SWAPFILE_PATH}.*|{SWAPFILE_PATH} none swap sw 0 0|' /etc/fstab
else
...
"""
    ...
    if ok:
        ...
        await msg.reply_text(
            "❌ Gagal membuat swap.\n"
            f"<code>{(proc.stderr or '')[-4000:]}</code>",
            parse_mode=ParseMode.HTML,
        )
...
sed -i '\\|{SWAPFILE_PATH} none swap sw 0 0|d' /etc/fstab || true
"""
